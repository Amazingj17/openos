...
成员 A 复核时应先在不查看 `optimal_cases.json` 的前提下手算并冻结至少 3 个案例，必须包含 G03、G05 和一个非对称通信案例；随后逐例核对 exact 结果、CPOP 调度合法性、超限失败路径和统一 evaluator 字段。A 签字前 P0-B03 保持“部分完成（提交复核）”。
