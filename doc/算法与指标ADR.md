...
| CI | smoke 仍为正态近似；P1-A03 对照新增固定双层配对 bootstrap；P1-B02 已冻结 5-seed 与异 seed 数 comparator 统计 | A 二次独立重算单策略/配对 bootstrap、逐 seed JSON/CSV 和四份 artifact 一致；B 已提交 OOD producer 候选 | A 独立复核 OOD source/hash/计时后，再进入 5-seed development/OOD |
...
P1-B02 已把下一阶段的主策略、reference、policy seed 数、ID/OOD 切片、失败惩罚、scheduler-only runtime、交叉分层 bootstrap、主策略配对比较、artifact hash 和一次性 test 工作流写入机器契约。首轮退回的逐 seed mean、receipt basename 和 UTC 三项已由 A [二次复核通过](./P1-B02第二轮独立复核记录.md)，契约/聚合器子阶段关闭。B 已提交 OOD materializer、冻结 development manifest 和只读 evidence producer 候选：30 个 ID 与三类各 30 个 OOD 场景已经真实物化，但没有运行任何策略；A 尚未独立复核，5-seed 主模型也未补齐，因此不能把“场景已生成”写成“泛化已验证”。
...
P1-B02 首轮历史见 [独立复核记录](./P1-B02独立复核记录.md)；B 修复后的逐 seed mean、receipt basename 和 UTC 已由 A 从 `071b861...` [二次复核通过](./P1-B02第二轮独立复核记录.md)，契约/聚合器子阶段关闭。B 现已提交确定性 OOD materializer/evidence producer 候选并冻结 development manifest；下一门禁是 A 从新不可变提交独立复核变换、source/hash、无 test 边界和 scheduler-only 计时。本阶段仍不读取公开 test，不启动新模型，也不运行 5-seed development/OOD。
