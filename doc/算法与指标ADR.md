...
| CI | smoke 仍为正态近似；P1-A03 对照新增固定双层配对 bootstrap；P1-B02 已冻结 5-seed 与异 seed 数 comparator 统计 | OOD source/计时和 R1/R2 通过；B 已统一 producer/report 排序 hash 并保留原 manifest 文件绑定 | A 从新提交复核后，再进入 5-seed development/OOD |
...
P1-B02 首轮历史见 [独立复核记录](./P1-B02独立复核记录.md)；B 修复后的逐 seed mean、receipt basename 和 UTC 已由 A 从 `071b861...` [二次复核通过](./P1-B02第二轮独立复核记录.md)，契约/聚合器子阶段关闭。OOD materializer/manifest、producer 外层计时和失败语义 R1/R2 已通过 A 的独立重算；B 已完成 producer/report ID hash 互操作候选，当前门禁为 A 从新不可变提交复核。本阶段仍不读取公开 test，不启动新模型，也不运行 5-seed development/OOD。
