...
P1-B02 将上述方法固化为机器契约：主策略和 Random 使用 5 seeds；确定性 baseline、单 seed BC 和既有 3-seed task-GNN 不复制行伪造相同 seed 数。主策略与 seed 数不同的 comparator 比较时，双方 seed 独立重采样、场景索引共享配对；只有 seed 集完全一致时才额外报告逐 seed×scenario 精确配对。完整定义见 [P1-B02 评测契约与自动报告](./P1-B02评测契约与自动报告.md)。
...
| CI | smoke 仍为正态近似；P1-A03 对照新增固定双层配对 bootstrap；P1-B02 已冻结 5-seed 与异 seed 数 comparator 统计 | task-GNN 10,000 次、固定 seed 的 95% CI 跨 0，未通过选择门禁；P1-B02 合成统计待 A 独立重算 | A 复核后，最终 5-seed development/OOD 沿用机器契约 |
...
| test 使用 | legacy `pipeline` 每次评测合成 test；公开 `train-bc/train-ppo` 对 test 设用途门禁；P1-B02 已有原子 receipt 和 final evidence 校验 | 正式学习运行未访问公开 test；当前 receipt 仍是本地同路径约束，不是数字签名，且尚未接到实际 loader | A 先复核 gate；随后把 claim 直接放到 public-test loader 之前并归档为不可覆盖 release artifact |
...
P1-B02 现已把下一阶段的主策略、reference、policy seed 数、ID/OOD 切片、失败惩罚、scheduler-only runtime、交叉分层 bootstrap、主策略配对比较、artifact hash 和一次性 test 工作流写入机器契约。当前仅有合成 fixture 证明报告器能接受合法 evidence 并拒绝常见漂移，尚无真实 OOD/test 结果，也未补齐 5-seed 主模型；A 独立复核通过前，不能把“协议已冻结”写成“泛化已验证”。
...
P1-B02 的候选契约、聚合器、CLI 和 16 项合成测试现已提交，详见 [P1-B02 评测契约与自动报告](./P1-B02评测契约与自动报告.md)。下一门禁是 A 从推送后的不可变提交独立手算、重采样和注入异常；通过后 B 才实现 OOD materializer/evidence producer。本阶段仍不读取公开 test，不启动新模型。
