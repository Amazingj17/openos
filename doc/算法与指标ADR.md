...
- 主责：成员 A
- 复核：成员 B
- 状态：已完成（P1-A05 正式负结果已双人复核；保留 P1-A04，G3/P1-06 不启动）
- 日期：2026-07-16
...
12. P1-A04 不重训既有三 seed：以固定来源 manifest `b4320d1b...f594` 逐字节继承 `20260717/18/19`，只训练 `20260720/21`。B 已从 detached `8692b61...` 在物理无 test 数据根正式预算复跑，五 seed validation mean/std `0.709190/0.064109`、零失败/非法，核心 artifact 与 A 包一致；P1-A04 关闭，允许开始正式 development/OOD。
13. P1-B02 正式 development 候选包含 17 个策略/seed 组合和 120 个冻结场景。A 已从 `origin/main=fab540c...` 的隔离 detached worktree 唯一正式复跑；A/B 去除 wall-clock 后的 2040 条调度记录逐字段一致，非时延报告完全一致，9/9 checkpoint 与 4/4 报告 artifact hash 均可重算。P1-B02 development/OOD 证据复核通过并关闭评测子阶段。
14. 证据通过不等于性能通过。P1-A04 MLP 的 ID/size/CCR/system mean ratio 为 `0.709190/1.568302/0.406325/0.965250`；size-OOD 五个 seed `1.601640/1.462686/1.610442/1.556301/1.610442` 均劣于 HEFT，system-OOD 配对 CI 跨 0。因此原样保留 `release_publishable=false`，不更换 seed、不临时调参、不访问 public test；当时只允许预注册一个 P1-A05 候选。
...
17. P1-A05 唯一候选冻结为等 transition 的 rollout-source 干预：每 PPO epoch 用 60×50-task STG + 30×100-task synthetic，恰为 `3000+3000=6000` transitions；五 seed×两 epoch 总预算仍为 60,000。14-D MLP、P1-A04 warm start、奖励、优化器、超参数和 ID checkpoint selection 全部不变；设计、实现、正式训练与评测全过程均未改变这些冻结项。
18. B 从远端不可变提交 `5857931...` 完成 `P1-A05-DESIGN-REVIEW`：A 报告 hash 可重建，60/60 场景内容 hash、性能与复合偏移重算一致，冻结 seed 重叠、91 episodes、6001 transitions、gamma、模型、public test 和训练调用共 7/7 注入被拒绝。该复核只授权实现；后续 60 场景物化去重、90/6000 dry-run、生产配置拒绝器和物理无 public-test 训练根也已实现并由 B 复核。
...
20. P1-A05 生产实现只允许预注册的逐 epoch 场景 ID 列表。实际隔离根为 120 train + 30 validation + 60 synthetic + 5 warm-start，四类 ID/hash 交集和 public-test/archive 均为 0；两轮 dry-run 均为 90/6000 且未加载 checkpoint/优化器。正式入口在输出创建前校验 B receipt，并要求干净工作树的受控训练源码与 receipt 的 `approved_source_commit` 零差异。receipt 进入远端祖先链后，A 在 `7d5cab6...` 上只执行一次正式训练，未使用 resume 或第二候选。
21. P1-A05 标准 development 报告用新 candidate 替换 `masked_mlp`，不能在同一契约里同时表示旧 P1-A04。只读比较器分别完整校验旧/新 evidence，再按相同五个 training seed 和相同 30 个 scenario 做精确配对，两层 bootstrap 共享 seed/scenario 重采样索引。正式结果中 size mean=`1.553568>1`，new−P1-A04 95% CI=`[-0.045177, 0.000027]`，机器决定保留 P1-A04 并停止 P1-A05。
22. B 在同一不可变候选上独立重算 50/50 training artifact、5/5 checkpoint、2040/2040 Development 记录、4/4 报告 artifact 和四切片 10,000 次 bootstrap，确认门禁与停止决定。Development 未全通过，因此不运行只属于通过分支的 scheduler replay、G3 或 P1-06；public test 与 `claim-test-gate` 均未触碰。
...
| CI | smoke 仍为正态近似；P1-A03 对照新增固定双层配对 bootstrap；P1-B02 冻结 5-seed 与异 seed 数 comparator 统计 | A 已从不可变提交独立重算；A/B 非时延报告一致，ID/CCR 有利、size 五 seed 全失败、system 跨 0 | 保留原报告和失败结论；新候选必须重新预注册并复核 |
...
| 训练算法 | legacy episodic REINFORCE；公开路径为 BC warm start + clipped PPO/GAE/value/target-KL | P1-A05 等 transition 的 50/100-task rollout 已完成唯一正式训练；五 seed×两 epoch、60,000 transitions，零 failure/illegal | 负结果已关闭；保留 P1-A04，不启动第二候选 |
...
| checkpoint/源码元数据 | checkpoint 自带维度/seed/特征；run manifest 记录配置、数据、代码、依赖和 checkpoint hash | P1-B03 的 `.gitattributes`、双 hash、canonical JSON、producer 迁移和旧 evidence 不改写已由 A 复核 | 保持 source bundle/checkpoint 双重溯源 |
| test 使用 | legacy `pipeline` 每次评测合成 test；公开 `train-bc/train-ppo` 对 test 设用途门禁；P1-B02 development materializer 固定只加载 `validation/model_selection` | P1-A05 evidence 记录 `test_accessed=false`、`public_test_loaded=false`；未运行 `claim-test-gate` | P1-A05 已因 size 门禁失败关闭，不得为本候选加载 public test |
...
P1-B02 已把主策略、reference、policy seed 数、ID/OOD 切片、失败惩罚、scheduler-only runtime、交叉分层 bootstrap、主策略配对比较、artifact hash 和一次性 test 工作流写入机器契约。契约/聚合器、OOD materializer/evidence producer、5-seed validation 包与 2040 条正式 development 证据均已双人复核。A/B 去除 wall-clock 后的调度行为与非时延报告一致，9 个 checkpoint 和 4 份报告 artifact 均可重算；完整证据见[正式结果](./P1-B02正式Development结果记录.md)和[正式独立复核](./P1-B02正式Development独立复核记录.md)。结果只支持“ID/CCR 有利、system 不确定、size 五 seed 全部失败”，不能写成“整体 OOD 泛化通过”；P1-B02 评测子阶段虽已关闭，`release_publishable=false` 与 G3 阻塞状态不变。

P1-A05 随后按冻结预算完成唯一正式训练和 Development。其 ID/size/CCR/system mean ratio 为 `0.703718/1.553568/0.419796/0.999317`；ID guardrail 通过，但 size 的绝对门禁和相对 P1-A04 的显著改善门禁失败。B 独立重算证据与 bootstrap 后确认停止决定，因此只允许表述为“等 transition 的 50/100-task rollout mixture 未解决 size-OOD 退化”；主模型继续使用 P1-A04。详见[正式负结果](./P1-A05正式负结果与停止记录.md)和[独立复核](./P1-A05正式负结果独立复核记录.md)。
...
9. **仍无公开 test 证据**：development ID/CCR/size/system OOD 已双签，但 public test 从未加载；开发结论不得冒充最终测试结论。
10. **泛化门禁明确失败**：P1-A04 的 size-OOD 五 seed 均劣于 HEFT；P1-A05 又未把 size mean 降到 1 以下，也未取得相对 P1-A04 的显著改善。两个候选均按冻结规则停止，不能靠重复试验挑结果。
...
6. A 已完成等 transition 的 50/100-task rollout 生产实现、物化去重、90/6000 dry-run 和无 public-test 训练根证明；B 从远端提交复核并签发绑定 receipt。（训练前实现已双签）
7. A 在不可变提交上完成唯一五 seed 正式训练和四切片 Development；size 两项门禁失败。B 独立重算负结果后关闭 P1-A05，保留 P1-A04；不启动 G3/P1-06/public test。（已关闭）
...
P1-B02 首轮历史见 [独立复核记录](./P1-B02独立复核记录.md)；契约/聚合器、OOD materializer/manifest、producer 外层计时、失败语义和 hash 互操作现均通过双人复核，完整终态见 [OOD 第三轮复核](./P1-B02OOD证据路径第三轮独立复核记录.md)。A 已按 [P1-A04 记录](./P1-A04五种子扩展设计与正式结果.md)只补两个缺失 seed，B 又在[独立复核](./P1-A04五种子扩展独立复核记录.md)中用物理无 test 根复跑通过。B 随后完成[正式 development 候选](./P1-B02正式Development结果记录.md)，A 从远端 `fab540c...` 唯一复跑并[正式复核通过](./P1-B02正式Development独立复核记录.md)。P1-A05 训练前实现和 P1-B03 均已交叉复核，随后唯一正式训练与 Development 也已完成；size 门禁失败并由 B 独立确认。历史 evidence 保持不变，主模型保留 P1-A04，G3/P1-06/public test 不启动。
