...
成员 B 已从远端不可变提交重跑 pipeline 和 checkpoint-only validation，核对非时延数字、checkpoint/training-history hash、结构化失败以及“synthetic smoke 不等于 public test”的边界。`summary.json` 因含绝对输出路径和 wall-clock，其 raw hash 跨 worktree 不固定；该边界已在[收口复核记录](./P0收口独立复核记录.md)明确。P0-09 已完成。
