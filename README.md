...
## P1-B02 评测契约与自动报告（B 已修复，待 A 二次复核）
...
本提交只有合成 fixture 验证，没有真实 OOD/test evidence。B 已针对[首轮独立复核记录](doc/P1-B02独立复核记录.md)中的 R1–R3 输出全部逐 seed mean 和 `evaluation_per_seed.csv`，在 claim/final 两处执行冻结 receipt basename，并将授权时间冻结为严格的 `YYYY-MM-DDTHH:MM:SSZ`。28 项专项测试与完整回归通过，但这些是 B 的自测，不替代 A 的二次独立复核；P1-B02 仍保持部分完成。实际 public-test loader 尚未接入，二次复核前不得访问公开 test。
...
- P1-B02 已冻结 ID/OOD、5-seed 和自动报告口径；B 已修复首轮退回的逐 seed mean、receipt basename 和 UTC 三项缺陷，待 A 二次复核后才实现 OOD materializer/evidence producer；
