...
task-GNN 已通过专用函数完成合成 4/2/0 数据的微型 1-seed BC/PPO：零失败、零非法动作，PPO 无改善时正确回退 BC warm start。完整 epoch 状态覆盖 actor/value、两套 Adam、双 RNG、history 和 best，微型连续/中断/恢复逐字段一致。它尚未接入 CLI run manifest、目录级 staging 和正式 3-seed validation，因此目前没有 GNN 性能结论，也没有新增公开 test 访问。冻结张量、状态格式、微型结果、验收门禁和下一提交要求见 [P1-A03 task-GNN 设计与验收契约](doc/P1-A03TaskGNN设计与验收.md)。
...
trisched/ppo.py          MLP/task-GNN 增量奖励、GAE/PPO 与 epoch 状态；MLP run 事务
...
- legacy `pipeline` 仍使用 REINFORCE；公开 STG 已有独立 masked PPO，task-GNN 仅完成合成微型训练/恢复，尚无 CLI、正式训练、课程或 OOD；
