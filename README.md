...
本提交只有合成 fixture 验证，没有真实 OOD/test evidence。A 的独立手算、bootstrap、失败分母、13 类拒绝路径、CLI 和 artifact hash 均通过，但发现报告未公开每个 seed mean，`receipt_name` 未参与 claim/final 校验，`authorized_at_utc` 接受任意非空字符串，因此 P1-B02 保持部分完成并退回 B。详见[首轮独立复核记录](doc/P1-B02独立复核记录.md)。实际 public-test loader 尚未接入，修复和二次复核前不得访问公开 test。
...
- P1-B02 已冻结 ID/OOD、5-seed 和自动报告口径，但 A 首轮复核退回逐 seed mean、receipt basename 和 UTC 三项缺陷；修复与二次复核后才实现 OOD materializer/evidence producer；
